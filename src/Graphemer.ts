...
// Optimization: Caches for property lookups
const breakPropertyCache = new Map<number, number>();
const emojiPropertyCache = new Map<number, number>();
const MAX_PROPERTY_CACHE_SIZE = 2000;

export default class Graphemer {
  // Optimization: Cache for break positions
  private static breakCache = new Map<string, Map<number, number>>();
  private static readonly MAX_STRING_CACHE_SIZE = 50;
  /**
  ...
  static nextBreak(string: string, index: number): number {
    ...
    // Optimization: Check cache for this string and index
    if (string.length <= 500) {
      const stringCache = Graphemer.breakCache.get(string);
      if (stringCache) {
    ...
    // Optimization: Fast path for simple ASCII characters
    const char = string.charCodeAt(index);
    if (
      char >= 0x20 &&
      char <= 0x7e &&
      !GraphemerHelper.isSurrogate(string, index)
    ) {
      const nextChar = string.charCodeAt(index + 1);
      if (
        nextChar >= 0x20 &&
        nextChar <= 0x7e &&
        !GraphemerHelper.isSurrogate(string, index + 1)
      ) {
        const result = index + 1;
        Graphemer.cacheBreak(string, index, result);
        return result;
      }
    }
    ...
    for (let i = index + 1; i < string.length; i++) {
      ...
      if (
        ...
        Graphemer.cacheBreak(string, index, i);
        return i;
      }
  ...
  /**
   * Cache a break position for a string
   * @private
   */
  private static cacheBreak(
    string: string,
    index: number,
    breakPos: number,
  ): void {
    ...
    let stringCache = Graphemer.breakCache.get(string);
    if (!stringCache) {
      if (Graphemer.breakCache.size >= Graphemer.MAX_STRING_CACHE_SIZE) {
        const firstKey = Graphemer.breakCache.keys().next().value;
        Graphemer.breakCache.delete(firstKey);
      }
      stringCache = new Map();
      Graphemer.breakCache.set(string, stringCache);
    }
    stringCache.set(index, breakPos);
  }
  ...
  static getGraphemeBreakProperty(code: number): number {
    ...
    const cached = breakPropertyCache.get(code);
    if (cached !== undefined) {
      return cached;
    }

    // Optimization: Fast path for common ASCII control characters
    if (code === 0x0a) {
      breakPropertyCache.set(code, CLUSTER_BREAK.LF);
      return CLUSTER_BREAK.LF;
    }
    if (code === 0x0d) {
      breakPropertyCache.set(code, CLUSTER_BREAK.CR);
      return CLUSTER_BREAK.CR;
    }
    if (code <= 0x1f || (code >= 0x7f && code <= 0x9f)) {
      breakPropertyCache.set(code, CLUSTER_BREAK.CONTROL);
      return CLUSTER_BREAK.CONTROL;
    }
    // Optimization: Fast path for common ASCII printable characters
    if (code >= 0x20 && code <= 0x7e) {
      breakPropertyCache.set(code, CLUSTER_BREAK.OTHER);
      return CLUSTER_BREAK.OTHER;
    }

    // Call the original implementation
    const result = Graphemer.getGraphemeBreakPropertyImpl(code);

    // Cache the result
    if (breakPropertyCache.size < MAX_PROPERTY_CACHE_SIZE) {
      breakPropertyCache.set(code, result);
    }
  ...
  /**
   * Original implementation of grapheme break property lookup
   * @private
   */
  private static getGraphemeBreakPropertyImpl(code: number): number {
  ...
  static getEmojiProperty(code: number): number {
    // Optimization: Check cache first
    const cached = emojiPropertyCache.get(code);
    if (cached !== undefined) {
      return cached;
    }

    // Call the original implementation
    const result = Graphemer.getEmojiPropertyImpl(code);

    // Cache the result
    if (emojiPropertyCache.size < MAX_PROPERTY_CACHE_SIZE) {
      emojiPropertyCache.set(code, result);
    }
  ...
  /**
   * Original implementation of emoji property lookup
   * @private
   */
  private static getEmojiPropertyImpl(code: number): number {
